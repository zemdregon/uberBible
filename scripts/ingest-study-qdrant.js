...
/**
 * Embed derived/study/jsonl into a separate Qdrant collection (uberbible_study).
 * Mirrors ingest-qdrant.js but targets study materials only.
 */
...
const STUDY_JSONL = path.join(ROOT, 'derived', 'study', 'jsonl');

const OLLAMA_HOST = (process.env.OLLAMA_HOST || 'http://127.0.0.1:11434').replace(/\/$/, '');
const OLLAMA_EMBED_MODEL = process.env.OLLAMA_EMBED_MODEL || 'nomic-embed-text';
const QDRANT_URL = (process.env.QDRANT_URL || 'http://127.0.0.1:6333').replace(/\/$/, '');
const COLLECTION = process.env.QDRANT_STUDY_COLLECTION || 'uberbible_study';
const BATCH = parseInt(process.env.INGEST_BATCH || '64', 10);
...
async function ensureCollection(vectorSize) {
  ...
  const indexes = [
    ['meta.work_id', 'keyword'],
    ['meta.kind', 'keyword'],
    ['meta.license', 'keyword'],
...
async function ingestFile(filePath) {
  ...
  for (let offset = 0; offset < records.length; offset += BATCH) {
    ...
    process.stdout.write(`\r${path.basename(filePath)}: ${upserted}/${records.length}`);
  }
...
async function main() {
  ...
  if (!fs.existsSync(STUDY_JSONL)) {
    console.error('No derived/study/jsonl — run: node scripts/normalize-study.js --all');
    process.exit(1);
  }
  ...
  if (args.all) {
    files = fs
      .readdirSync(STUDY_JSONL)
      .filter((f) => f.endsWith('.jsonl'))
      .map((f) => path.join(STUDY_JSONL, f))
      .sort();
  } else if (args.ids.length) {
    files = args.ids.map((id) => path.join(STUDY_JSONL, `${id}.jsonl`));
    ...
    console.error('Usage: node scripts/ingest-study-qdrant.js --id strongs-hebrew | --all');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('Start e.g.: cd .qdrant && steam-run ~/.local/bin/qdrant');
    process.exit(1);
  }
  ...
  console.log(`Done. Upserted ${total} study points.`);
}
